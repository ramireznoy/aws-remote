...
// Look up an environment object by name (throws if not found)
...
// Resolve {env} and {repo} placeholders in a name pattern
function resolveName(pattern, envName, repoName) {
  return (pattern || '').replace('{env}', envName).replace('{repo}', repoName || '');
}

module.exports = { loadConfig, saveConfig, getClient, getEnvObject, resolveName, getAvailableProfiles, validateConfigProfile, CONFIG_PATH };
